...
namespace LogbookManagementService.Controllers
{
    ...
    public class LogbookController : ControllerBase
    {
        ...
        [HttpGet("WTGTop10Error")]
        public IActionResult WTGTop10Error(string TurbineNo)
        {
            _logger.LogInformation("WTGTop10Error  method started");
            try
            {
                var response = _logbookService.WTGTop10Error(TurbineNo);
                if (response.data == null)
                {
                    _logger.LogError("no data exist.");
                }
            ...
            {
                _logger.LogError("an error occurred while executing WTGTop10error method.");
                return StatusCode(500, "an error occurred while executing WTGTop10error method.");
            }
        }
        ...
        [HttpGet("ManualWTGLogbook")]
        public IActionResult ManualWTGLogbook(string siteName)
        {
            {
                _logger.LogInformation("GetWTGLogbook Method Started");
                try
                {
                    var response = _logbookService.ManualWTGLogbook(siteName);
                    if (response.data == null)
                    {
                        _logger.LogError("Details of logbook  not exist.");
                    }
        ...
        [HttpGet("GetWTGKpiMA_GA")]
        public IActionResult GetWTGKpiMA_GA(string TurbineNo)
        {
            try
            {
                _logger.LogInformation("GetWTGKpiMA_GA Method Started");
                var response = _logbookService.GetWTGKpiMA_GA(TurbineNo);
            ...
            {
                _logger.LogError("An error occurred while getting WTG KPI Details!");
                return StatusCode(500, "An error occurred while getting WTG KPI Details.");
            }
        }
        ...
        [HttpGet("GetWTGKpiMTTR_MTBF")]
        public IActionResult GetWTGKpiMTTR_MTBF(string TurbineNo)
        {
            try
            {
                _logger.LogInformation("WTG KpiMTTR_MTBF Method Started");
                var response = _logbookService.GetWTGKpiMTTR_MTBF(TurbineNo);

                if (response.data == null)
                {
                    _logger.LogInformation("no WTG KpiMTTR_MTBF");
                }
                return Ok(response);
            }
            catch (Exception)
            {
                _logger.LogError("An error occurred while retrieving WTG KpiMTTR_MTBF");
                return StatusCode(500, "An error occurred while retrieving WTG KpiMTTR_MTBF.");
            }
        }
        ...
        [Authorize]
        public IActionResult LogbookSubmitButton(string? ShiftCycle, string? SiteName, DateTime LogDate, string? status, bool GridBreakdownNoActivity, bool ScadaNoActivity)
        {
            try
            {
                ...
                var response = _logbookService.LogbookSubmitButton(ShiftCycle, SiteName, LogDate, status, userName, GridBreakdownNoActivity, ScadaNoActivity);
