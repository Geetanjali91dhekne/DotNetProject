...
namespace LogbookManagementService.Repositories
{
    public class LogbookRepository : ILogbookRepository
    {
               ...
               //string URI = "http://172.16.11.74:8090/api/SuzlonActiveUser/IsUserActive";
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (userResult != null && userResult.IsActive)
                    {
                        ...
                        response.Email = userResult.EmailId;
                        return response;
                    }
                    else
                    {
            ...
            try
            {
                ...
                if (data.Count == 0)
                {
                    ...
                    foreach (var list in siteTurbine)
                    {
                        ...
                        var records = (from mainRecord in _szFleetMgrContext.VwWindFarms
                                       join turbinerecord in _szFleetMgrContext.CatalogTurbines
                                       on mainRecord.PlantUnitId equals turbinerecord.PlantUnitId
                                       join eventrecord in _szFleetMgrContext.TurbineOnlines
                    ...
                    if (liveData.Count == 0)
                    {
                        ...
                        //merging of history and manual 

                        var allData = AllWTGLogbook(siteName);

                        if (allData != null)
                        {
                            var turbineList = new HashSet<string>(response.TurbineLists.Select(t => t.Turbine));
                            var errorList = new HashSet<string>(response.ErrorLists.Select(e => e.Error));

                            response.TurbineLists.AddRange(allData.TurbineLists.Where(t => !turbineList.Contains(t.Turbine)));
                            response.ErrorLists.AddRange(allData.ErrorLists.Where(e => !errorList.Contains(e.Error)));
                ...
                foreach (var listData in data)
                {
                    ...
                    {
                        var plantIdtoEventCode = _szFleetMgrContext.TurbineOnlines
        ...
        public ResponseModelLogbook ManualWTGLogbook(string siteName)
        {
            try
            {
                var model = new ResponseWTGLogbookList();

                List<TurbineList> TurbineLists = new List<TurbineList>();
                List<ErrorList> ErrorLists = new List<ErrorList>();

                var sapFunctionLocationCode = _crmsContext.VScAllMachineStaticDetailOmsPbis
                    .Where(i => i.MainSite == siteName)
                    .Select(i => i.SapFuncLocCode)
                    .OrderBy(i => i)
                    .ToList();

                var errorName = _szFleetMgrContext.ErrorMasters
                    .Select(i => i.Name)
                    .OrderBy(i => i)
                    .Distinct()
                    .ToList();

                foreach (var location in sapFunctionLocationCode)
                {
                    TurbineList data = new TurbineList();
                    data.Turbine = location;
                    TurbineLists.Add(data);
                }
                foreach (var error in errorName)
                {
                    ErrorList data = new ErrorList();
                    data.Error = error;
                    ErrorLists.Add(data);
                }

                model.TurbineLists = TurbineLists;
                model.ErrorLists = ErrorLists;

                int countWTGLogbook = sapFunctionLocationCode.Count;

                return GetResponseModelLogbook(Constants.httpCodeSuccess, model, "Data Received for manual filling", true, false, countWTGLogbook);
            }
            catch (Exception ex)
            {
                return GetResponseModelLogbook(Constants.httpCodeFailure, null, ex.Message, false, false, 0);
        ...
        private ResponseWTGLogbookList AllWTGLogbook(string siteName)
        {
            var model = new ResponseWTGLogbookList();
      ...
      public ResponseModel WhyReason()
        ...
        public ResponseModel GetWTGKpiMA_GA(string TurbineNo)
        {
            try
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "sp_MA_GA_WTG_KPI_Data";
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@TurbineNo", TurbineNo));
        ...
        public ResponseModel GetWTGKpiMTTR_MTBF(string TurbineNo)
        {
            try
            {
                ...
                using (var cmd = conn.CreateCommand())
                {
                    cmd.CommandText = "sp_WTG_KPI_MTTR_MTBF";
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.Add(new SqlParameter("@TurbineNo", TurbineNo));
            ...
            try
            {
                    ...
                    else if (checkCRMSBreakdown.Count != 0 && WTGAnalysisList.Count != 0)
                    {
                        foreach (var detail2 in WTGAnalysisList)
                        {
                            ...
                            if (existingAnalysis2 != null)
                            {
                                ...
                                newWhyAnalysis.PendingHrs = CalculatePendingHrs(detail2.GrandTotal, existingAnalysis2.Id);
                            ...
                            else
                            {
                               ...
                               .FirstOrDefault(d => d.MainSite == detail2.Site && d.SapFuncLocCode == detail2.SapCode);
                                ...
                                if (updateAnalysis2 != null)
                                {
                                    ...
                                    newWhyAnalysis.PendingHrs = CalculatePendingHrs(detail2.GrandTotal, 0);
                        ...
                        foreach (var detail in checkCRMSBreakdown)
                        {
                            ...
                            if (existingAnalysis != null)
                            {
                                ...
                                newWhyAnalysis.PendingHrs = CalculatePendingHrs(detail.TotalDuration.ToString(), existingAnalysis.Id);
                            ...
                            else
                            {
                               ...
                               .FirstOrDefault(d => d.MainSiteCode == detail.MainSiteCode && d.SapFuncLocCode == detail.SapFuncLocCode);
                                ...
                                if (updateAnalysis != null)
                                {
                                    ...
                                    newWhyAnalysis.CheckMark = true;
                                    newWhyAnalysis.Status = "";
                                    newWhyAnalysis.PendingHrs = CalculatePendingHrs(detail.TotalDuration.ToString(), 0);
                    ...
                    else if (WTGAnalysisList.Count != 0)
                    {
                        foreach (var detail2 in WTGAnalysisList)
                        {
                            ...
                            if (existingAnalysis != null)
                            {
                                ...
                                newWhyAnalysis.SubBucket = existingAnalysis.SubBucket;
                                newWhyAnalysis.CheckMark = true;
                                newWhyAnalysis.PendingHrs = CalculatePendingHrs(detail2.GrandTotal, existingAnalysis.Id);
                ...
                if (getWeek != null)
                {
                    ...
                    var query = from record in _crmsContext.CrmBreakdownDetails
                                where record.DateOfGen >= startOfWeek && record.DateOfGen <= endOfWeek
        ...
        private float CalculatePendingHrs(string? grandTotal, int id)
        {
            float totalHrs = 0;
            float pendingHrs = 0;
            ...
            {
                var hrsInString = data.Hrs;
                if (float.TryParse(hrsInString, out float hrsInInteger))
                {
                    totalHrs += hrsInInteger;
                }
            }

            if (float.TryParse(grandTotal, out float grandTotalInInteger))
            {
                pendingHrs = grandTotalInInteger - totalHrs;
            }
        ...
        public ResponseModel WTGTop10Error(string TurbineNo)
        {
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    using (SqlCommand command = new SqlCommand("sp_get_wtg_error", connection))
                    {
                        command.CommandType = CommandType.StoredProcedure;

                        command.Parameters.AddWithValue("@TurbineNo", TurbineNo);
            ...
            try
            {
                ...
                if (existingLogbookWTGBreakdownDetails == null)
                {
                    ...
                    if (getTurbinefromCRMS != null)
                    {
                        ...
                        {
                            var windFarmDataFromDatamart = _szFleetMgrContext.VwWindFarms
                                .FirstOrDefault(d => d.PlantUnitId == getPlantIdFromDatamart.PlantUnitId);
                            if (windFarmDataFromDatamart != null)
                            {
                                var turbineDataFromDatamart = _szFleetMgrContext.TurbineOnlines
        ...
        public ResponseModel LogbookSubmitButton(string? ShiftCycle, string? SiteName, DateTime LogDate, string? status, string? userName, bool GridBreakdownNoActivity, bool ScadaNoActivity)
        {
            try
            {
                ...
                foreach (var list in existingStatus)
                {
                    ...
                    list.ModifiedDate = DateTime.Now;
                    list.ModifiedBy = userName;
                    list.GridBreakdownNoActivity = GridBreakdownNoActivity;
                    list.ScadaNoActivity = ScadaNoActivity;
                }
