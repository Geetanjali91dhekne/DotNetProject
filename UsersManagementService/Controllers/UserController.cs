...
namespace UsersManagementService.Controllers
{
    ...
    public class UserController : ControllerBase
    {
        ...
        [HttpPost("SendEmail")]
        public IActionResult SendEmail([FromForm] string? templatename,  [FromForm] int tableid, [FromForm] int taskid)
        {
            try
            {
                _logger.LogInformation("GetAreaByStateCode Method Started");
                var response = _context.SendEmail(templatename, tableid, taskid);
                if (response.data == null)
                {
                    _logger.LogInformation(" Details Not Added");
                }
