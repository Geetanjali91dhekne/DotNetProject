...
namespace UsersManagementService.Repositories
{
    public class TaskRepository : ITaskRepository
    {
        ...
        public TaskRepository(FleetManagerContext fleetManagerContext, CrmsContext crmsContext, IUserRepository userRepository)
        {
        ...
        {
            var siteList = fleetManagerContext.TUserSiteMappings.Where(d => d.EmployeeCode == userName).Select(d => d.SiteCode).ToList();
            ...
            try
            {
                //var taskList = fleetManagerContext.TaskMains.Where(d => siteList.Contains(d.SiteCode) || d.CreatedBy == userName || d.ModifiedBy == userName || d.SiteIncharge == userName).ToList();
                var taskList = fleetManagerContext.TaskMains.AsEnumerable().Where(d => siteList.Contains(d.SiteCode) && (d.CreatedBy == userName || d.ModifiedBy == userName || d.SiteIncharge == userName || (!string.IsNullOrEmpty(d.AssignedToUser) && d.AssignedToUser.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim()).Contains(userName)) || (!string.IsNullOrEmpty(d.Reviewer) && d.Reviewer.Split(new[] { ',' }, StringSplitOptions.RemoveEmptyEntries).Select(s => s.Trim()).Contains(userName)))).ToList();

                var totalRecords = 0;
                var pageSize = 20;

                List<int> statusList = !string.IsNullOrEmpty(status) ? status.Split(',').Select(s => fleetManagerContext.TaskManagementStatuses.FirstOrDefault(d => d.Status == s.Trim()).Id).ToList() : new List<int>();

                if (statusList.Any())
                {
                    taskList = taskList.Where(d => statusList.Contains(d.StatusId)).OrderBy(x => x.PriorityId).ThenBy(x => x.DueDate).ThenByDescending(x => x.ModifiedDate).ToList();

                }
                totalRecords = taskList.Count;

                var responseList = new List<GetTaskMain>();
                int validRecordsCount = 0;


                if (!string.IsNullOrEmpty(searchQuery) || !string.IsNullOrEmpty(taskType) || !string.IsNullOrEmpty(sprint) || !string.IsNullOrEmpty(priority) || !string.IsNullOrEmpty(natureOfTask))
                {
                    responseList = taskList.Select(list => ConvertToResponseTaskMain(list, view))
                        .Where(result => result != null).ToList();

                }
                else
                {
                    responseList = taskList.Skip((pageNo - 1) * pageSize).Where(task =>
                     {
                         var result = ConvertToResponseTaskMain(task, view);
                         if (result != null)
                         {
                             validRecordsCount++;
                             return validRecordsCount <= 20;
                         }
                         return false;
                     }).Select(list => ConvertToResponseTaskMain(list, view))
                     .Where(result => result != null).Take(pageSize).ToList();
                }



                //var userTaskList = fleetManagerContext.TaskMains.Where(d => d.CreatedBy == userName || d.ModifiedBy == userName || d.SiteIncharge == userName).ToList();
                ...
                if (!string.IsNullOrEmpty(searchQuery))
                {
                    ...
                    task.SiteName?.IndexOf(searchQuery, StringComparison.OrdinalIgnoreCase) >= 0
                ).ToList();
                    totalRecords = responseList.Count;

                }
                int noOfPages = (int)Math.Ceiling(Convert.ToDecimal(totalRecords) / Convert.ToDecimal(pageSize));
                if (!string.IsNullOrEmpty(searchQuery) || !string.IsNullOrEmpty(taskType) || !string.IsNullOrEmpty(sprint) || !string.IsNullOrEmpty(priority) || !string.IsNullOrEmpty(natureOfTask))
                {
                    responseList = responseList.Skip((pageNo - 1) * pageSize).Take(pageSize).ToList();
                }
                if (responseList.Count == 0)
                {
                    return GetResponseModel(Constants.httpCodeSuccess, null, "Task Not Found", false, 0, 0);
                }

                return GetResponseModel(Constants.httpCodeSuccess, responseList, "All Tasks Listed", true, noOfPages, totalRecords);
            }
            catch (Exception ex)
            {
                return GetResponseModel(Constants.httpCodeFailure, null, ex.Message, false, 0, 0);
            }
        }
        /*
        public ResponseModel GetAllTasks(string? userName,string? searchQuery = null, string? view = null, string? status = null, string? taskType = null,
        ...
        {            
            ...
            try
            {
                var taskList = fleetManagerContext.TaskMains.Where(d => siteList.Contains(d.SiteCode) || d.CreatedBy == userName || d.ModifiedBy == userName || d.SiteIncharge == userName)
                    .OrderBy(d => d.Sequence).Take(10).ToList();
		...
		*/
        public ResponseModel GetAllTaskById(int? id)
        {
            try
            {
                ...
                var taskData = ConvertToResponseTaskMainId(task);
        ...
        private GetTaskMain ConvertToResponseTaskMain(TaskMain task, string? view)
        {
            var currentDate = DateTime.Now;
            var taskstatus = fleetManagerContext.TaskManagementStatuses.FirstOrDefault(d => d.Id == task.StatusId);

            if (view.Equals("Kanban"))
            {
                var status = taskstatus?.Status;
                if ((status != "TO DO" && status != "DONE") ||
                    (status == "TO DO" && task.DueDate <= CalculateDueDate(currentDate, taskstatus.AllowedDays, "TO DO")) ||
                    (status == "DONE" && task.StatusChangedDate >= CalculateDueDate(currentDate, taskstatus.AllowedDays, "DONE")))
                {

                }
                else
                {
                    // If the task doesn't meet Kanban conditions, return null or a default value
                    return null;
                }
            }
            ...
            foreach (string assignpart in assignedToUserParts)
            {
                ...
                userAssigned.EmpCode = assignpart;//empDetails?.Code;
                userAssigned.EmpDomainId = assignpart;// empDetails?.Code;
                userAssigned.EmpName = assignpart;// empDetails?.Name;
            ...
            foreach (string reviewpart in reviewerToUserParts)
            {
                ...
                revAssigned.EmpCode = reviewpart;// empDetails?.Code;
                revAssigned.EmpDomainId = reviewpart;// empDetails?.Code;
                revAssigned.EmpName = reviewpart;// empDetails?.Name;
                reviewer.Add(revAssigned);
            }
		...
		*/
        private GetTaskMainId ConvertToResponseTaskMainId(TaskMain task)
        {
            string[] assignedToUserParts = task.AssignedToUser?.Split(',')?.Select(s => s.Trim()).ToArray() ?? Array.Empty<string>();
            string[] reviewerToUserParts = task.Reviewer?.Split(',')?.Select(s => s.Trim()).ToArray() ?? Array.Empty<string>();
            ...
            return new GetTaskMainId
            {
                ...
                Sequence = task.Sequence,
                SiteIncharge = task.SiteIncharge,
                SiteCode = crmsContext.ScMainSites.FirstOrDefault(d => d.MainSiteCode == task.SiteCode)
            };
        }

        public async Task<ResponseModel> AddorUpdateTasks(ResponseTaskMain responseTaskMain, string? username)
        {
            try
            {
                int affect = 0;
                ...
                if (existingTask == null)
                {
                    ...
                    newTask.SiteIncharge = username;

                    var siteInchargeDetails = GetUserDetails(username);
                    string? siteInchargeEmail = null;
                    string? siteInchargeName = null;
                    if (siteInchargeDetails != null)
                    {
                        siteInchargeEmail = siteInchargeDetails.EmailId;
                        siteInchargeName = siteInchargeDetails.Name;
                    }

                    newTask.SiteInChargeEmail = siteInchargeEmail;
                    newTask.SiteInchargeName = siteInchargeName;
                ...
                else
                {
                    ...
                    //fleetManagerContext.SaveChanges();
                    affect = await fleetManagerContext.SaveChangesAsync();

                    if (affect != 0)
                    {
                        var temp = "Task Allocate to user";
                        var tableid = 0;
                        var jobId = BackgroundJob.Enqueue(() => _userRepository.SendEmail(temp, tableid, responseTaskMain.Id));

                        //return GetResponseModel(Constants.httpCodeSuccess, existingTask, "Task Updated Successfully and email sent", true);
                    }
            ...
            try
            {
                ...
                foreach (var task in allTasks)
                {
                    ...
                    if (existingTask == null)
                    {
                        ...
                        if (newTask == null)
                        {
                            return GetResponseModel(Constants.httpCodeSuccess, null, "Task Not Added as there are no AssignedUser or Reviewer", false);
                        }
                    ...
                    else
                    {
                        ...
                        //if (updatedTask == null)
                        //{
                        //    return GetResponseModel(Constants.httpCodeSuccess, task, "Task Not Updated as there are no AssignedUser or Reviewer", false);
                        //}
        ...
        public async Task<ResponseModel> UpdateTaskBoardNew(updateTaskMngrModel task, string? username)
        {
            try
            {
                int affect = 0;
                ...
                if (existingTask == null)
                {
                    return GetResponseModel(Constants.httpCodeFailure, null, "Task not found.", false);

                }
                var status = fleetManagerContext.TaskManagementStatuses.FirstOrDefault(t => t.Status == task.Status);

                if (status == null)
                {
                    return GetResponseModel(Constants.httpCodeFailure, null, "Status not found.", false);

                }
                existingTask.StatusId = status.Id;
                existingTask.ModifiedBy = username;
                existingTask.ModifiedDate = DateTime.Now;
                existingTask.StatusChangedBy = username;
                existingTask.StatusChangedDate = DateTime.Now;

                fleetManagerContext.TaskMains.Update(existingTask);
                affect = await fleetManagerContext.SaveChangesAsync();
                ...
                if (affect != 0)
                {
                    var temp = "Task Allocate to user";
                    var tableid = 0;
                    var jobId = BackgroundJob.Enqueue(() => _userRepository.SendEmail(temp, tableid, task.Id));

                    //return GetResponseModel(Constants.httpCodeSuccess, existingTask, "Task Updated Successfully and email sent", true);
                }

                return GetResponseModel(Constants.httpCodeSuccess, existingTask, "Task Updated Successfully", true);
            }
            catch (Exception ex)
            {
                return GetResponseModel(Constants.httpCodeFailure, null, ex.Message, false);
        ...
        {
            List<string>? AssignedToEmailArray = new List<string>();

            List<string>? AssignedToNameArray = new List<string>();

            foreach (var item in responseTaskMain.AssignedToUser)
            {
                var data = GetUserDetails(item);
                if (data != null)
                {
                    AssignedToNameArray.Add(data.Name);
                    AssignedToEmailArray.Add(data.EmailId);
                }
            }
            string? AssignedToEmail = string.Join(',', AssignedToEmailArray);
            string? AssignedToName = string.Join(',', AssignedToNameArray);


            List<string>? ReviewerEmailArray = new List<string>();

            List<string>? ReviewerNameArray = new List<string>();

            foreach (var item in responseTaskMain.Reviewer)
            {
                var data = GetUserDetails(item);
                if (data != null)
                {
                    ReviewerNameArray.Add(data.Name);
                    ReviewerEmailArray.Add(data.EmailId);
                }
            }
            string? ReviewerEmail = string.Join(',', ReviewerEmailArray);
            string? ReviewerName = string.Join(',', ReviewerNameArray);
            ...
            newTaskMain.AssignedToName = AssignedToName;
            newTaskMain.AssignedToEmail = AssignedToEmail;
            newTaskMain.ReviewerEmail = ReviewerEmail;
            newTaskMain.ReviewerName = ReviewerName;
        ...
        private TaskMain MapTasks(TaskMain existingTask, ResponseTaskMain updateTask)
        {
            ...
            foreach (var item in updateTask.AssignedToUser)
            {
                var data = GetUserDetails(item);
                if (data != null)
                {
            ...
            foreach (var item in updateTask.Reviewer)
            {
                var data = GetUserDetails(item);
                if (data != null)
                {
            ...
            var siteInchargeDetails = GetUserDetails(updateTask.SiteIncharge);
            string? siteInchargeEmail = null;
            string? siteInchargeName = null;
            if (siteInchargeDetails != null)
            {
            ...
            existingTask.ReviewerEmail = ReviewerEmail;
            existingTask.ReviewerName= ReviewerName;
            existingTask.SiteInchargeName = siteInchargeName;
            existingTask.SiteInChargeEmail = siteInchargeEmail;
            existingTask.AssignedToEmail  = AssignedToEmail;
        ...
        private TaskMain CreateTaskOrder(SequenceTaskMng responseTaskMain)
        {
            ...
            foreach (var list in responseTaskMain.Reviewer)
            {
                ...
                string reviewerList = list;
                if (!string.IsNullOrEmpty(reviewerList))
                    ReviewerCodes.Add(reviewerList);
            }

            if (EmpCodes.Count > 0 && ReviewerCodes.Count > 0)
            {
                TaskMain newTaskMain = new TaskMain();
        ...
        private TaskMain MapTaskOrder(TaskMain existingTask, SequenceTaskMng updateTask)
        {
            ...
            if (existingTask.StatusId != updateTask.StatusId.Id)
            {
                if (EmpCodes.Count > 0 && ReviewerCodes.Count > 0)
                {
                    existingTask.TicketNo = updateTask.TicketNo;
            ...
            else if (existingTask.Sequence != updateTask.Sequence)
            {
                existingTask.TicketNo = updateTask.TicketNo;
                existingTask.Title = updateTask.Title;
                existingTask.Description = updateTask.Description;
                    ...
                    for (int i = 0; i < model.file.Count; i++)
                    {
                        if (model.mandate_id != null && model.entityname != null && model.file != null && model.RecordId != null)
                        {
                            ...
                            if (model.entityname == "MasterUpload")
                            {
                                ...
                                //string folderpath = @"D:\FleetManager\upload\bulkUploadFile\";
        ...
        public void SendMailsforAutoTask()
        {
            Console.WriteLine("Entered In SendMailsforAutoTask");
            var list = fleetManagerContext.TaskMains.Where(x=> x.CreatedBy == "Auto" && !string.IsNullOrEmpty(x.SiteIncharge) && string.IsNullOrEmpty(x.SiteInChargeEmail)).ToList();
            Console.WriteLine($"Got List {list.Count}");

            foreach (var item in list)
            {
                Console.WriteLine($"Inside loop {item.Id}");

                var siteInchargeDetails = GetUserDetails(item.SiteIncharge);
                string? siteInchargeEmail = null;
                string? siteInchargeName = null;
                if (siteInchargeDetails != null)
                {
                ...
                item.SiteInChargeEmail = siteInchargeEmail;
                item.SiteInchargeName = siteInchargeName;
                fleetManagerContext.TaskMains.Update(item);
                fleetManagerContext.SaveChanges();

            }
            Console.WriteLine("Checking for task not yet mailed");
            var taskForMailing = fleetManagerContext.TaskMains.Where(x => x.CreatedBy == "Auto" && x.IsMailSent == false).ToList();
            Console.WriteLine($"Total task for mailing {taskForMailing.Count}");

            foreach (var task in taskForMailing)
            {
                Console.WriteLine($"Sending mail for TaskId: {task.Id}");

                var temp = "Task Allocate to user";
                var tableid = 0;
                var jobId = BackgroundJob.Enqueue(() => _userRepository.SendEmail(temp, tableid, task.Id));
                task.IsMailSent = true;
                fleetManagerContext.TaskMains.Update(task);
                fleetManagerContext.SaveChanges();
            }
        }
            ...
            try
            {
                ...
                using (WebClient wc = new WebClient())
                {
                    ...
                    if (userResult != null && userResult.IsActive)
                    {
                        ...
                        response.EmailId = userResult.EmailId;
                        return response;
                    }
                    else
                    {
        ...
        private ResponseModel GetResponseModel
            (int code, object? data, string message, bool status, int? PageCount = 0, int? totalRecords = 0)
        {
            responseModel.code = code;
            responseModel.data = data;
            responseModel.message = message;
            responseModel.status = status;
            responseModel.totalPageCount = PageCount;
